...
//      abajo. El avatar la reproduce en bucle mientras no hay ninguna seña activa, y
//      hace un fundido hacia/desde ella al empezar o terminar una seña — sin este clip,
//      el avatar se queda en la pose de referencia del modelo (T-pose) en reposo, y la
//      transición hacia la primera seña se ve como un manotazo extraño.
...
export default function AvatarGLTF({ animacion, url = '/modelos/avatar.glb' }) {
  ...
  useEffect(() => {
    ...
    accion.fadeIn(DURACION_FUNDIDO).play();
    if (anterior && anterior !== accion) anterior.fadeOut(DURACION_FUNDIDO);
    accionActivaRef.current = accion;

    return () => {
      accion.fadeOut(DURACION_FUNDIDO);
      const reposo = actions[CLIP_REPOSO];
      if (reposo) {
        reposo.reset().setLoop(THREE.LoopRepeat, Infinity).fadeIn(DURACION_FUNDIDO).play();
        accionActivaRef.current = reposo;
      }
    };
